...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors([{ field: 'general', message: 'Failed to create account. Please try again.' }])
    } finally {
      setIsLoading(false)
    }
  }
            ...
            <div>
              ...
              <select
                ...
                className={`appearance-none rounded-lg relative block w-full px-3 py-3 border ${
                  getFieldError('userType') ? 'border-red-300' : 'border-gray-300'
                } placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm`}
              ...
              {getFieldError('userType') && (
                <p className="mt-1 text-sm text-red-600">{getFieldError('userType')}</p>
              )}
            </div>
